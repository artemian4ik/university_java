...
public class VectorTest extends TestCase {
	...
	public void testValues () {
		Integer [] expented = new Integer[3];
		for (int e = 0; e < expented.length; e++) {
			expented[e] = new Integer(e + 1);
		}
		Integer [] actual = (Integer[]) fFull.toArray(new Integer[fEmpty.size()]);
		assertEquals(expented.length, actual.length);
		for (int i = 0; i < actual.length; i++) {
			assertEquals(expented[i], actual[i]);
		}
	}
}